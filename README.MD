...
```js
app.use('/api/v1/authenticateUser', authRouter);
```
...
- URL global var
- JOBIFY Collection
- authenticateUser and jobs folders
- setup routes
...
const registerUser = async (currentUser) => {
  ...
  try {
    const response = await axios.post('/api/v1/authenticateUser/register', currentUser);
...
const loginUser = async (currentUser) => {
  ...
  try {
    const { data } = await axios.post('/api/v1/authenticateUser/login', currentUser);
...
const setupUser = async ({ currentUser, endPoint, alertText }) => {
  ...
  try {
    const { data } = await axios.post(`/api/v1/authenticateUser/${endPoint}`, currentUser);
...
- create authenticateUser.js in <b>middleware</b>
...
const updaterUser = async (currentUser) => {
  try {
    const { data } = await axios.patch('/api/v1/authenticateUser/updateUser', currentUser, {
      headers: {
...
const updaterUser = async (currentUser) => {
  try {
    const { data } = await authFetch.patch('/authenticateUser/updateUser', currentUser);
  } catch (error) {
    console.log(error.response);
  }
};
...
- check for test user in authenticateUser middleware
...
const getCurrentUser = async () => {
  ...
  try {
    const { data } = await authFetch('/authenticateUser/getCurrentUser');
    const { user, location } = data;
...
```js
const logoutUser = async () => {
  await authFetch.get('/authenticateUser/logout');
  dispatch({ type: LOGOUT_USER });
};
```
